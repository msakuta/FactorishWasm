...
pub(crate) fn item_to_str(type_: &ItemType) -> String {
    match type_ {
        ...
        ItemType::Spoilage => "Spoilage".to_string(),
...
pub(crate) fn str_to_item(name: &str) -> Option<ItemType> {
    match name {
        ...
        "Spoilage" => Some(ItemType::Spoilage),
...
pub(crate) fn render_drop_item(
    ...
    match item_type {
        ...
        ItemType::Spoilage => render16(&state.image_spoilage),
...
pub(crate) fn render_drop_item_mat_gl(
    ...
    match item_type {
        ...
        ItemType::Spoilage => render16(&state.assets.tex_spoilage),
...
pub(crate) fn get_item_image_url<'a>(state: &'a FactorishState, item_type: &ItemType) -> &'a str {
    match item_type {
        ...
        ItemType::Spoilage => &state.image_spoilage.as_ref().unwrap().url,
